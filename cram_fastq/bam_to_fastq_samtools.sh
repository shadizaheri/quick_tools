...
# This is the RECOMMENDED approach - faster and simpler
...
docker run --rm -i \
    ...
    -c "
        set -euo pipefail
        pigz -p $THREADS -c > /output/${OUTPUT_BASE}_1.fastq.gz &
        pigz -p $THREADS -c > /output/${OUTPUT_BASE}_2.fastq.gz &
        pigz -p $THREADS -c > /output/${OUTPUT_BASE}_unpaired.fastq.gz &
        wait
    "
