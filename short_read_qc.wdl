...
workflow QCWorkflow {
    ...
    # Step 1: Collect sequencing yield metrics
    call CollectQualityYieldMetrics {
        input:
            input_bam = input_bam,
            metrics_filename = "quality_yield_metrics.txt",
            preemptible_tries = preemptible_tries
    }

    # Step 2: Collect insert size and base quality metrics
    call CollectUnsortedReadgroupBamQualityMetrics {
        input:
            input_bam = input_bam,
            output_bam_prefix = "quality_metrics",
            preemptible_tries = preemptible_tries
    }

    # Step 3: Collect WGS coverage metrics
    call CollectWgsMetrics {
        input:
            input_bam = input_bam,
            input_bam_index = input_bam_index,
            metrics_filename = "wgs_metrics.txt",
    ...
    output {
        ...
        File insert_size_metrics = CollectUnsortedReadgroupBamQualityMetrics.insert_size_metrics
        File insert_size_histogram_pdf = CollectUnsortedReadgroupBamQualityMetrics.insert_size_histogram_pdf
